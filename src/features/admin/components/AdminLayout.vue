...
const openMenu = ref('users'); // Puede ser 'users', 'server', o null
  ...
  <div class="font-body-md overflow-x-hidden bg-[#f9f9f9] text-[#1a1c1c] min-h-screen">
    ...
    <aside class="fixed left-0 top-0 z-50 flex h-full w-64 flex-col gap-4 border-r border-[#EDEDED] bg-white p-5 font-['Inter'] text-sm antialiased">
      ...
      <nav class="flex flex-col gap-2 overflow-y-auto">
        <!-- Dashboard Principal -->
        <RouterLink to="/admin" active-class="bg-[#F7F7F5] text-black font-semibold" class="flex items-center gap-3 rounded-md px-3 py-2 text-black transition-all hover:bg-[#F7F7F5] active:scale-[0.98]">
        ...
        <div>
          ...
          <div v-show="openMenu === 'users'" class="ml-8 mt-1 flex flex-col gap-1 border-l-2 border-[#EDEDED] pl-2">
            <button @click="showUnavailableFeature('Todos los usuarios')" class="text-left px-2 py-1.5 text-[13px] text-[#49473f] hover:text-black hover:bg-[#F7F7F5] rounded">Todos</button>
            <button @click="showUnavailableFeature('Usuarios Activos (Approved)')" class="text-left px-2 py-1.5 text-[13px] text-[#49473f] hover:text-black hover:bg-[#F7F7F5] rounded">Aprobados / Activos</button>
            <button @click="showUnavailableFeature('Usuarios Pendientes')" class="text-left px-2 py-1.5 text-[13px] text-[#49473f] hover:text-black hover:bg-[#F7F7F5] rounded flex justify-between items-center">
              Pendientes
              <span class="bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded text-[10px] font-bold">Nuevos</span>
            </button>
            <button @click="showUnavailableFeature('Usuarios Baneados')" class="text-left px-2 py-1.5 text-[13px] text-[#49473f] hover:text-black hover:bg-[#F7F7F5] rounded">Baneados</button>
            <button @click="showUnavailableFeature('Peticiones Rechazadas')" class="text-left px-2 py-1.5 text-[13px] text-[#49473f] hover:text-black hover:bg-[#F7F7F5] rounded">Rechazados</button>
            <button @click="showUnavailableFeature('Usuarios Eliminados')" class="text-left px-2 py-1.5 text-[13px] text-red-600 hover:bg-red-50 rounded">Eliminados</button>
          </div>
        </div>
      ...
      <header class="flex justify-between items-center px-8 h-14 bg-white/80 backdrop-blur-md border-b border-[#EDEDED] sticky top-0 z-40 font-['Inter'] text-sm tracking-tight">
        <div class="flex items-center gap-4 w-1/3">
          <div class="relative w-full max-w-sm">
            ...
            <input @click="showUnavailableFeature('Búsqueda global de sistema')" class="w-full bg-[#F7F7F5] border-none rounded-[4px] py-1.5 pl-9 pr-4 text-sm focus:ring-1 focus:ring-secondary/20 placeholder:text-[#37352F]/30" placeholder="Search users, settings..." type="text" />
          </div>
        </div>
